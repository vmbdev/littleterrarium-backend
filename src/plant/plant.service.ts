  ...
  ltFindMany(
    ...
    if (options.filter) {
      query.where = {
        ...query.where,
        sortName: {
          contains: prepareForSortName(options.filter),
        },
      };
    }
    ...
    if (options.sort) {
      ...
      const nullOrder = options.order === 'asc' ? 'last' : 'first';
      query.orderBy = [];
      ...
      } else query.orderBy.push({ createdAt: order });
      
      query.orderBy.push({ id: order });
    }
